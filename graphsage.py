...
class GraphSage(tf.keras.Model):
    def __init__(self, raw_features, internal_dim, num_layers):
        ...
        for i in range(1, num_layers + 1):
            ...
            input_dim = internal_dim if i > 1 else 60
        ...
        self.dense1 = Dense(64, activation=tf.nn.relu, name="dense1")
        self.dense1.build(input_shape=(None, internal_dim * 2))

        self.dense3 = Dense(8, activation=tf.nn.relu, name="dense3")
    ...
    def train(self, src_nodes0, dstsrc2src0_1, dstsrc2src0_2, dstsrc2dst0_1, dstsrc2dst0_2, dif_mat0_1, dif_mat0_2,
        ...
        with tf.GradientTape() as tape:
            predict_value = self.call(src_nodes0, dstsrc2src0_1, dstsrc2src0_2, dstsrc2dst0_1, dstsrc2dst0_2,
                                      ...
                                      src_nodes1, dstsrc2src1_1, dstsrc2src1_2, dstsrc2dst1_1, dstsrc2dst1_2,
                                      dif_mat1_1, dif_mat1_2)
            loss = self.compute_uloss(predict_value, true_bandwidth)

        grads = tape.gradient(loss, self.trainable_weights)
        self.optimizer.apply_gradients(zip(grads, self.trainable_weights))
        return loss

    def compute_uloss(self, predict_value, true_bandwidth):
        """
        loss function 需要魔改，这三者维度相同
        :param true_bandwidth: 真实带宽
        :param predict_value: 预测值
        :return: 平均损失函数
        """
        # print(true_bandwidth)
        # print(predict_value)
        # print(tf.cast(predict_value, dtype=tf.float64))
        # return tf.subtract(true_bandwidth, tf.cast(predict_value, dtype=tf.float64))
